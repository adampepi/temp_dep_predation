...
o =0.003
h =0.1
## From lab rearing -- death rate per day at Topt
K=100 ##much more realistic K
parms2 <- c(a,c,m,b,g,n,o,h,K )
...
predprey_equations_DD_JD  <- function(t, y, parms) {
  ...
  dJdt <- b*A0*(1-(J0+A0)/K) - g*J0 - a*J0*P0/(1+a*h*J0) -o*J0  #juvenile ode
...
same<-tempresponse2(ts,ToptJ=25,ToptP=25,TsdA=0.005,TsdP=0.005,RmP=.1,RmJ=.4,Rmo=.3,Rmn=.3,Rmm=.1, parms=parms,ODE=predprey_equations_DD_JD)

same

str(same)

preym1<-tempresponse2(ts,ToptJ=25,ToptP=23,TsdA=0.005,TsdP=0.005,RmP=.1,RmJ=.4,Rmo=.3,Rmn=.3,Rmm=.1, parms=parms, ODE=predprey_equations_DD_JD)
preym1
plot(preym1$Ps)
preyp1<-tempresponse2(ts,ToptJ=25,ToptP=27,TsdA=0.005,TsdP=0.005,RmP=.1,RmJ=.4,Rmo=.3,Rmn=.3,Rmm=.1, parms=parms, ODE=predprey_equations_DD_JD)
###Base model plot
